...
class DownloadEngine {
  ...
  static Future<void> _runJob(
    ...
    for (int i = 0; i < chapterNumbers.length; i++) {
      ...
      final label = formatChapterLabel(chapterNum);
      ...
      try {
        final chapterUrl = await _resolveChapterUrl(adapter, seriesUrl, chapterNum);
        ...
        try {
          for (int j = 0; j < images.length; j++) {
            ...
            await dataSource.downloadToFile(images[j], filePath);
  ...
  static Future<String> _resolveChapterUrl(
    BaseSiteAdapter adapter,
    String seriesUrl,
